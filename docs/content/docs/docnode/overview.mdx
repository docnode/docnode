...
```ts
import { Doc, string, defineNode } from "@docukit/docnode";
...
// New doc that accepts MyNodeDef
const doc = new Doc({
  extensions: [{ nodes: [MyNodeDef] }],
...
doc.dispose(); // Release and lock the document to be garbage-collected
...
// For configuration options, see https://docukit.dev/docnode/undo-manager

doc.undoManager.undo(); // Undo the last undoable transaction
doc.undoManager.redo(); // Redo the last undone transaction
doc.undoManager.canUndo(); // Check if there is an undo step available
doc.undoManager.canRedo(); // Check if there is a redo step available
doc.undoManager.isEnabled; // Check if maxUndoSteps is greater than 0
